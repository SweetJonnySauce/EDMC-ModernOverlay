...
- Run the overlay in DEV mode by updating `version.py` and making sure `__version__` ends with `-dev` (e.g., `0.7.4-dev`). With this set, dev mode is auto-enabled.
- Alternatively, export `MODERN_OVERLAY_DEV_MODE=1` before launching EDMC (e.g., `export MODERN_OVERLAY_DEV_MODE=1` on Linux; set the env var in a terminal/PowerShell on Windows).
- Verify startup: the EDMC log will include `Running Modern Overlay dev build (...)`.
- In dev mode, `debug.json` flags take effect (tracing, payload mirroring, repaint logging, etc.). Make sure `capture_client_stderrout` is `true`
