...
| Force-render monitor thread (`ModernOverlayForceMonitor`) | `_start_force_render_monitor_if_needed` `load.py:805-835`, invoked from `__init__` `load.py:427-428` when `allow_force_render_release` is true | Only `Event` set in `stop` (`_force_monitor_stop.set()` `load.py:522`); no join/handle clear | Starts pre-`start()`; unjoined thread; relies on flag flip or event to exit; handle remains. |
